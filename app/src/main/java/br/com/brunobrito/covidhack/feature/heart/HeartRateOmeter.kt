...
import net.kibotu.heartrateometer.MathHelper
...
open class HeartRateOmeter {
    ...
    fun bpmUpdates(context: Context, surfaceHolder: SurfaceHolder): Observable<Bpm> {
    ...
    private fun getScreenDimensions(): Dimension {
        ...
        val display: android.view.Display = (context?.get()
        ...
        if (Build.VERSION.SDK_INT in 14..16) {
            try {
                ...
                screenHeight = android.view.Display::class.java.getMethod("getRawHeight")
        ...
        return object : Camera.PreviewCallback {
            ...
            override fun onPreviewFrame(data: ByteArray?, camera: Camera?) {
                ...
                try {
                    ...
                    val x =
                        toPrimitive(sampleQueue.toArray(arrayOfNulls<Double>(0)) as Array<Double>)
    ...
    fun stop(c: Context) {
        ...
        cameraSupport?.parameters?.setFlashMode(Camera.Parameters.FLASH_MODE_OFF)
        turnOffFlashLight(c)
    }

    open fun turnOffFlashLight(c: Context) {
        try {
